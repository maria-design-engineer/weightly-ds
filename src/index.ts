...
// Библиотека объявляет себя браузерной, и это её граница, а не граница экрана.
// Причина: пакет собирается одним файлом, и семь компонентов из сорока шести
// стоят на Base UI, который держит состояние через React-контекст. Контекста
// на сервере нет, и серверный компонент падает на любом импорте отсюда — хоть
// на разметочном Label.
//
// Веса это не меняет: в браузер уезжает ровно то же самое, что и при пометке
// на стороне продукта. Меняется место, где сказано «дальше браузер»: здесь
// один раз, а не на каждом экране заново — решение пользователя 10.09.2026.
//
// Данные при этом остаются на сервере: страница читает их серверным
// компонентом и отдаёт сюда пропсами. Ставить рядом с ними запросы к базе
// или секреты нельзя — всё помеченное уезжает к человеку целиком.
