...
export function ExerciseCard({
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      /*
       * Минимум карточки — всё, кроме названия, плюс две строки и разделитель. От числа
       * видимых строк не зависит: так высота обрезанного названия не упирает карточку
       * в прежний размер.
       */
      const without = card.clientHeight - available
      card.style.setProperty('--w-exercise-card-min', `${Math.ceil(without + MIN_TITLE_LINES * step + 1)}px`)
